...
exit 0
# This script implements SSH hardening measures.
...
harden_ssh() {
    log_action "Starting SSH hardening process."

    # Disable root login
    if $DRY_RUN; then
        log_action "DRY RUN: Would disable root login in /etc/ssh/sshd_config."
    else
        sed -i.bak 's/^PermitRootLogin yes/PermitRootLogin no/' /etc/ssh/sshd_config
        log_action "Disabled root login in /etc/ssh/sshd_config."
    fi

    # Change default SSH port
    if $DRY_RUN; then
        log_action "DRY RUN: Would change SSH port to 2222 in /etc/ssh/sshd_config."
    else
        sed -i.bak 's/^#Port 22/Port 2222/' /etc/ssh/sshd_config
        log_action "Changed SSH port to 2222 in /etc/ssh/sshd_config."
    fi

    # Restart SSH service
    if $DRY_RUN; then
        log_action "DRY RUN: Would restart SSH service."
    else
        systemctl restart sshd
        log_action "Restarted SSH service."
    fi

    log_action "SSH hardening process completed."
}
