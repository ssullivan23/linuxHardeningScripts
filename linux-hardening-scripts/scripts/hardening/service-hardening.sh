...
# Purpose: Disable unnecessary services and secure running services
...
if [ "$DRY_RUN" = true ]; then
    log_info "=== SERVICE HARDENING (DRY RUN MODE) ==="
else
...
audit_running_services() {
    ...
    local running_services=$(systemctl list-units --type=service --state=running --no-pager --no-legend 2>/dev/null | wc -l)
